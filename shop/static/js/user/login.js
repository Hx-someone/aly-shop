$(function () {
    let $loginName = $(".name_input"); //获取用户登录名输入框
    ...
    $login.submit(function (arg) {
        ...
        //获取参数
        let $userAccount = $("input[name=username]").val();//获取用户登录名输入值
        let $pwd = $("input[name=pwd]").val();  //获取用户密码输入框
            ...
            success:function (res) {
                if(res.errno==="0"){
                    ...
                    setTimeout(function(){
                        ...
                        window.location.href = '/goods/index/';  // 登录成功后跳转到index页面
